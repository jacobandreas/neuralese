...
class RolloutPlaceholders(object):
    def __init__(self, task, n_hidden, n_code):
        t_x = []
        t_h = []
        t_z = []
        t_q = []
        for i_agent in range(2):
            ...
            t_h.append(tf.placeholder(tf.float32, (1, n_hidden)))
            t_z.append(tf.placeholder(tf.float32, (1, n_code)))
            t_q.append(tf.zeros((1, task.n_actions), tf.float32))
        self.t_x = tuple(t_x)
        self.t_h = tuple(t_h)
        self.t_z = tuple(t_z)
        self.t_q = tuple(t_q)
...
class ReplayPlaceholders(object):
    def __init__(self, task, n_hidden, n_code, n_batch, n_history):
        self.t_reward = tf.placeholder(tf.float32, (n_batch, n_history))
        self.t_terminal = tf.placeholder(tf.float32, (n_batch, n_history))
        self.t_mask = tf.placeholder(tf.float32, (n_batch, n_history))
        ...
        for i_agent in range(2):
            t_x.append(tf.placeholder(
                    tf.float32, (n_batch, n_history, task.n_features)))
            ...
            t_action.append(tf.placeholder(
                    tf.float32,
                    (n_batch, n_history, task.n_actions)))
        self.t_x = tuple(t_x)
